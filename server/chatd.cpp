...
UDPsocket udpSocket;       /* Socket descriptor */
...
void sendOutUDPs(char* msg, int channel){
    ...
    for (int i=0; i<GAME_MAXPEOPLE; ++i ) {
		...
		if ( people[i].active && c != id.c_str()) {
        	...
        	p->address.host =  people[i].peer.host;    /* Set the destination host */
        	p->address.port =  people[i].peer.port;    /* And destination port SDL_SwapBE16(7777);*/
